...
export function CatalogoSalaoScreen({ navigation, route }) {
  const { id } = route.params || {}; // Recebe id do usuário, evita erro se não vier

  useEffect(() => {
    if (!id) {
      // Se não tiver id, redireciona para Login (ou outra tela que desejar)
  ...
  useEffect(() => {
    const buscarEmpresasSalao = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/cadastroempresas?categoria=Salão"
        );
  ...
  return (
    <View style={styles.fullContainer}>
      <ScrollView contentContainerStyle={[styles.container, { flexGrow: 1 }]}>
        ...
        <Text style={styles.sectionTitle}>Catálogo de Vendas - Salão</Text>
