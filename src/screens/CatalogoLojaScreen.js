...
export function CatalogoLojaScreen({ navigation, route }) {
  const { id } = route.params || {};

  useEffect(() => {
    if (!id) {
      navigation.navigate("Login"); // Ajuste para sua tela de login ou inicial
    }
  }, [id]);
  ...
  useEffect(() => {
    const buscarEmpresasLoja = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/cadastroempresas?categoria=Loja"
        );
  ...
  return (
    <View style={styles.fullContainer}>
      <ScrollView contentContainerStyle={[styles.container, { flexGrow: 1 }]}>
        ...
        <Text style={styles.sectionTitle}>Catálogo de Vendas - Loja</Text>
