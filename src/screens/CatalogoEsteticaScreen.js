...
export function CatalogoEsteticaScreen({ navigation, route }) {
  const { id } = route.params || {};

  useEffect(() => {
    if (!id) {
      navigation.navigate("Login"); // ou outra tela inicial, se desejar
    }
  }, [id]);
  ...
  useEffect(() => {
    const buscarEmpresasEstetica = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/cadastroempresas?categoria=Estética"
        );
  ...
  return (
    <View style={styles.fullContainer}>
      <ScrollView contentContainerStyle={[styles.container, { flexGrow: 1 }]}>
        ...
        <Text style={styles.sectionTitle}>Catálogo de Vendas - Estética</Text>
