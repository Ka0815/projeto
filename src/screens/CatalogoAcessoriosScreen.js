...
export function CatalogoAcessoriosScreen({ navigation, route }) {
  const { id } = route.params || {};

  useEffect(() => {
    if (!id) {
      navigation.navigate("Login"); // redireciona se não tiver id, ajuste conforme sua tela inicial
    }
  }, [id]);
  ...
  useEffect(() => {
    const buscarEmpresasAcessorios = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/cadastroempresas?categoria=Acessórios"
        );
  ...
  return (
    <View style={styles.fullContainer}>
      <ScrollView contentContainerStyle={[styles.container, { flexGrow: 1 }]}>
        ...
        <Text style={styles.sectionTitle}>Catálogo de Vendas - Acessórios</Text>
