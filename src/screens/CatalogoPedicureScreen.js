...
export function CatalogoPedicureScreen({ navigation, route }) {
  const { id } = route.params || {}; // Recebe o id do usuário
  ...
  useEffect(() => {
    const buscarEmpresasPedicure = async () => {
      try {
        const response = await axios.get(
          "http://localhost:3000/cadastroempresas?categoria=Pedicure"
        );
  ...
  return (
    <View style={styles.fullContainer}>
      <ScrollView contentContainerStyle={[styles.container, { flexGrow: 1 }]}>
        ...
        <Text style={styles.sectionTitle}>Catálogo de Vendas - Pedicure</Text>
